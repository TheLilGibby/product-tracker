...
def first_json_ld_product(soup):
    """Return the first schema.org Product object from JSON-LD scripts, or None."""
...
def _find_json_ld_product(data):
    ...
    if any(t == 'Product' for t in type_list):
        return data
    if '@graph' in data:
        return _find_json_ld_product(data['@graph'])
    return None


def json_ld_offers(product):
    """Normalize Product.offers to a list of Offer dicts."""
...
def json_ld_price(product):
    """First numeric price on a JSON-LD Product, or None."""
    for offer in json_ld_offers(product):
        raw = offer.get('price')
        if raw in (None, ''):
            continue
        try:
...
def json_ld_available(product):
    """
    Availability from schema.org Offer.availability.

    InStock / LimitedAvailability / PreOrder count as orderable. OutOfStock
    and SoldOut do not. Returns None when no offer spells it out.
    """
    orderable = ('instock', 'limitedavailability', 'preorder')
    unavailable = ('outofstock', 'soldout', 'discontinued', 'offlineonly')
    for offer in json_ld_offers(product):
        raw = str(offer.get('availability') or '').lower()
        if any(token in raw for token in unavailable):
            return False
        if any(token in raw for token in orderable):
            return True
    return None


def json_ld_image(product):
    """First image URL on a JSON-LD Product, or None."""
    ...
    if isinstance(image, list) and image:
        ...
        if isinstance(first, dict):
            return (first.get('url') or first.get('contentUrl') or '').strip() or None
    if isinstance(image, dict):
        return (image.get('url') or image.get('contentUrl') or '').strip() or None
    return None
